...
var playAddr = "http://webclient.dzcatv.com/application/newPlay/play.php";
}else{
var playAddr = "http://webclient.dzcatv.com/application/browser/newPlay/play.php";
}
...
(function(){
	...
	var globalPos=function(){
		...
		this.get = function(){
			var index=arguments[0];
			//addByYangcl
			iDebug("new_common.js---globalPos()---get()--index="+index);
			var str = "";
			var tmp = "";
			if(this.getBrowser()=="iPanel"){
				iDebug("common.js--get()--this.getBrowser():"+this.getBrowser());
				if(iPanel.eventFrame.__globalPos){
					str = iPanel.eventFrame.__globalPos[index];	
					tmp = str;
				iDebug("common.js--get()-ipanel-tmp:"+tmp);
				}
			}else{
				if(window.localStorage){
					iDebug("common.js--get()--window.localStorage:"+window.localStorage);
					str = window.localStorage.getItem(index);
					iDebug("common.js--get()-localStorage-str:"+str);
					if(str) tmp = str;
				    iDebug("common.js--get()-localStorage-tmp:"+tmp);
				}else{
					str= this.getCookie(index);
					// addByYangcl
					iDebug("new_common.js---globalPos()---get()--str="+str);
					if(str.indexOf("|") != -1){
					  tmp = str.replace("#0#0#", "|");
					}
					tmp = str;
					// addByYangcl 
					iDebug("new_common.js---globalPos()---get()--tmp="+tmp);
				}
			}	
		...
		this.getCookie=function(_name){
			...
			// addByYangcl
			iDebug("new_common.js---globalPos()---getCookie()--url"+url);
			if(new RegExp(".*\\b"+_name+"\\b(\\s*=([^&;]+)).*", "gi").test(url)){
				iDebug("new_common.js---globalPos()---getCookie()--unescape(RegExp.$2)"+unescape(RegExp.$2));
