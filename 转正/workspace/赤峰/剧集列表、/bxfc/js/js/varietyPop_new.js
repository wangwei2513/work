...
(function(){
	...
	var container=function(args){
		...
		this.initPopData = function(){
			...
			iDebug("TVPop_new.js----this.initData====this.varietyPopdata.isType:"+ (typeof this.varietyPopdata.isType));
			if(typeof this.varietyPopdata.isType!="undefined" && this.varietyPopdata.isType=="vod")	 this.showChannelFlag = 0;
			if(this.showChannelFlag){
				...
				iPanel.debug("---yanch here="+this.varietyPopdata.textArr[0]+";;--yanch textName="+this.textName+";;;text="+this.varietyPopdata.textArr[0].substring(0,this.getRealLength(this.varietyPopdata.textArr[0],20)))
				this.$(this.textName).innerHTML = '<table id="'+this.textName+1+'" cellpadding="0" width="100%" height="100%" style="padding-left:10px;font-size:24px;color:'+this.containInfo.divTop.color+'; font-family:'+this.containInfo.fontFamily+';"><tr height="60"><td id="currPrograme" style="font-size:'+this.containInfo.divTop.fontSize+';">'	+this.varietyPopdata.textArr[0].substring(0,this.getRealLength(this.varietyPopdata.textArr[0],20))+'</td>	</tr></table>';
				...
				if(tmpNum < this.dataLength){
					...
					/*if(i ==  this.pageSize - 1){
						...
						this.$(this.tdName+i).style.color = this.containInfo.td.tdcolor;
						 ...
						 var _date = '<span id = "'+this.tdName+i+'_1" style="color:'+this.containInfo.td.tdcolorS+'; font-size:'+this.containInfo.td.sizeS+';padding-top:6px; padding-left:10px;">'+this.varietyPopdata.data[speacial].name+"&nbsp;"+(this.varietyPopdata.data[speacial].program_name.substring(0,this.getRealLength(this.varietyPopdata.data[speacial].program_name,15))+(this.getLength(this.varietyPopdata.data[speacial].program_name)>15?"...":""))+'</span>';
		...
		this.left = function(){
			if(this.showChannelFlag){
				var leng = this.varietyPopdata.chanListArr.length;
				if(leng==0||(leng<=2&&this.channelFocus == 0)){
					return;
				}
				if(this.focusArea==0){
					this.focusArea = 1;
		...
		this.right = function(){
			if(this.showChannelFlag){
				var leng = this.varietyPopdata.chanListArr.length;
				if(leng==0||(leng<=2&&this.channelFocus == leng-1)){
					return;
				}
				if(this.focusArea==0){
					this.focusArea = 1;
		...
		this.doSelect = function(){
			...
			if(this.focusArea==0){
				...
				iDebug("[varietyPop_new.js]---doSelect()---isPlayPageFlag="+isPlayPageFlag+",chanId="+chanId+",listLabel="+this.varietyPopdata.listLabel);
				iPanel.eventFrame.openPage(playAddress,{type:"play",isMode:"channelId",channelId:chanId,listLabel:this.varietyPopdata.listLabel,isAppFlag:1});	
...
this.getStrChineseLen=function(){
				var arr = str.split("");
	var n = arr.length;
	var i = 0;
	for(i = n - 1; i >= 0; i--){
	...
	return str; 
		...
		this.goToBackTv = function(){
			...
			iDebug("-varietyPop.js---programId:"+programId+",listLabel="+this.varietyPopdata.listLabel);
            ...
            if(!this.isiPanel){
				...
				var __params = {
					...
					programId:programId,
					actionPA:19,//选集看
					listLabel:this.varietyPopdata.listLabel,
					backUrl:PlayBackUrl
				}
