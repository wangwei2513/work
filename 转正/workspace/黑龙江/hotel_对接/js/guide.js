...
function eventHandler (eventObj) {
  iDebug('[guide.js]__keycode=' + eventObj.code)
...
function changeUD (_num) {
  switch (focusArea) {
    case 0: // 左侧栏目
      ...
      break
    case 1: // 右侧内容
      // if(listPos+currPage*pageSize<3+_num*3<0||listPos+currPage*pageSize+_num*3>contentList.length-1)return
      ...
      scroll_Bar.scroll(listPos + currPage * pageSize)
      break
    case 2: // 右侧内容的下一级
...
function changeLR (_num) {
  switch (focusArea) {
    case 0: // 左侧栏目
      if (_num > 0 && contentList.length > 0) {
        loseFocus()
        focusArea = 1
        onFocus()
      }
...
function changePage (_num) {
  ...
  iDebug('[guide.js]__currPage=' + currPage + '__listPos=' + listPos)
...
function getColumnList () {
  ...
  ajaxObj.successCallback = function (_xmlHttp) {
    iDebug('[guide.js]__getColumnList__successCallback !')
  ...
  ajaxObj.failureCallback = function (_xmlHttp) {
    iDebug('[guide.js]__getColumnList__failureCallback !')
  }
...
function getContent () {
  ...
  // ajaxObj.charset = "gbk"
  ajaxObj.successCallback = function (_xmlHttp) {
    iDebug('[guide.js]__getContent__successCallback !')
    var jsonData = eval('(' + _xmlHttp.responseText + ')')
  ...
  ajaxObj.failureCallback = function (_xmlHttp) {
    iDebug('[guide.js]__getContent__failureCallback !')
  }
...
function initContent () {
  if (focusArea == 2) {
    ...
    $('contentTxt').innerHTML = contentList[listPos + currPage * pageSize].intro
    ...
    fillList()
    showScroll(listPos + currPage * pageSize, contentList.length)
  }
}
