    ...
    var Page = TVUI.Class.create({
        ...
        events: function () {
            ...
            this.registerEvent(Event.onComKey([Key.NUM9, Key.NUM6, Key.NUM9, Key.NUM5, Key.NUM6], function () {
                if (!this._active) return;
                this.go("ui://factory/templates.html");
            }, this));
...
(function (TVUI) {
    ...
    var channel = {
        ...
        saveOffChannel: function (content, path) {
            ...
            api.SysSetting.env(path, content);
        ...
        getOffChannel: function (path) {
            var channel = null;
            var str = api.SysSetting.env(path);
            if (str) {
                channel = eval('(' + str + ')');
            }
            /*
...
/**
 *  页面状态机, 给ETV使用, 因为在ETV不能通过TVUI.Page做状态控制
 * @type {*}
 */
TVUI.PageState = TVUI.Class.create({
...
(window.$V || ($V = {})).config = {
    ...
    //环境变量
    variable: {
        icNo: TVUI.API.CA.icNo || '8002002601147041',
        account: TVUI.API.DataAccess.info('UserInfo', 'account') || 'kenny'
    },
    ...
    themes: {
        'none': {
            name: 'none',
            desc: '空白主题',
            href: 'css/themes/none/style.css',
            preview: 'css/themes/none/preview.png'
        },
        'red': {
            name: 'red',
            desc: '红色主题',
            href: 'css/themes/red/style.css',
            preview: 'css/themes/red/preview.png'
        },
        'blue': {
            name: 'blue',
            desc: '蓝色主题',
            href: 'css/themes/blue/style.css',
            preview: 'css/themes/blue/preview.png'
        },
        'grey': {
            name: 'grey',
            desc: '灰色主题',
            href: 'css/themes/grey/style.css',
            preview: 'css/themes/grey/preview.png'
        }
    },
...
(function ($V) {
    ...
    $V.modules = {
         ...
         */
        get: function (id) {
            return id ? this.__modules[id] : this.__modules;
        },
         ...
         */
        newId: function () {
            return 'mod_' + (new Date()).getTime();
        },
        /**
         * 缓存操作函数
         * @param key
         * @param value
         * @param saveToEnv 是否保存到环境变量
         * @returns {*}
         */
        cache: function (key, value, saveToEnv) {
            //设置缓存
            if (key && value) {
                this.__caches[key] = value;
                saveToEnv && TVUI.API.SysSetting.envJSON(key, value);

            } else if (key && value === null) { //删除缓存
                saveToEnv && TVUI.API.SysSetting.envJSON(key, null);
                delete  this.__caches[key];

            } else if (key) { //获取缓存
                return this.__caches[key] || TVUI.API.SysSetting.envJSON(key) || null;
            } else {
                return this.__caches;
            }
        },
        /**
         * 注入环境变量
         ...
         * @param obj
         */
        inject: function (content, obj) {
            return TVUI.View.render(content, $.extend(true, {}, obj || {}, {variable: $V.config.variable}));
        }
    };


})(window.$V || ($V = {}));
(function ($V, mod, $, TVUI, exports) {


    //扩展TVUI按钮组件
    TVUI.Button = TVUI.Class.create({
        init: function (options) {
            var o = this.options = $.extend({
                ...
                //向上事件激活按钮实例或window下的按钮实例名称
                up: null,

                //向上事件激活按钮实例或window下的按钮实例名称
                down: null,

                //向左事件激活按钮实例或window下的按钮实例名称
                left: null,

                //向右事件激活按钮实例或window下的按钮实例名称
                right: null
            }, options || {});
        ...
        active: function () {
            ...
            this.parent.prototype.active.apply(this, arguments);
            this.focus();
        },
        unActive: function () {
            this.parent.prototype.unActive.apply(this, arguments);
            this.blur();
        },
        select: function () {
            this.fire('select');
        ...
        events: function () {
            ...
            this.registerEvent(TVUI.Event.on('keyup', function (e) {
                ...
                switch (e.which) {
                    case TVUI.Key.LEFT:
                        this.fire('left', {type: 'left'});
                        break;
                    case TVUI.Key.RIGHT:
                        this.fire('right', {type: 'right'});
                        break;
                    case TVUI.Key.UP:
                        this.fire('up', {type: 'up'});
                        break;
                    case TVUI.Key.DOWN:
                        this.fire('down', {type: 'down'});
                        break;
                    case TVUI.Key.SELECT:
                        this.select();
                        break;
                }
            ...
            this.on('left right up down', function (e) {
                var s = o[e.type], obj = window[s];
    ...
    //TVUI走马灯字幕组件
    TVUI.Marquee = TVUI.Class.create({
        init: function (options) {
            var o = this.options = $.extend({
                //组件容器
                el: document.body,

                //滚动方向,有4种值: left right up down
                direction: 'left',

                //滚动方式,有两种值: scroll slide , scroll是单向循环滚动,slide只滚动一次
                behavior: 'scroll',
            ...
            this.el = $(o.el);
            this.marquee = document.createElement('marquee');
        },
        setContent: function (content) {
            this.marquee.innerHTML = content;
        },
        render: function () {
            var o = this.options, marquee = this.marquee;
            $.each(o, function (key, value) {
                if (key != 'el' && key != 'content') {
                    marquee.setAttribute(key, value);
                }
            });

            this.setContent(o.content);
            this.el[0].appendChild(marquee);
        },
        start: function () {
            this.marquee.start();
            this.fire('start');
        ...
        destroy: function () {
            this.el[0].removeChild(this.marquee);
            this.parent.prototype.destroy.apply(this, arguments);
        }
    }, TVUI.Base);
    ...
    var Base = TVUI.Class.create({
        ...
        init: function (config) {
            ...
            }, $V.config.modules[this.type] || {}, config || {});

            //组件标识id， 如果参数无传人id，即新建一个唯一id
            this.id = o.id ? o.id : mod.newId();
            this.type = this.type || o.type;
            this.parentId = null;

            //读取环境变量存起来
            this.variable = $.extend(true, {}, $V.config.variable);

            //如果参数有id并且组件是静态化的，说明html已经生成在页面了，否则创建容器html
            this.el = (o.id && o.isStatic) ? $('#' + o.id) : $('<div id="' + this.id + '"></div>');
         ...
         * @param o
         */
        update: function (config, isInit) {
            var o = this.config = $.extend(this.config, config || {});
            ...
            this.isCache = o.isCache;
            this.style = o.style;
            this.options = o.options;
            this.autoRender = o.autoRender;
            this.appendTo = o.appendTo;

            if (this.isStatic) {
                this.viewData = {
                    data: o.api ? this.ajax() : this.data,
            ...
            //导出实例对象到window
            exports[this.name] = this;

            !isInit && this.fire('update', o);
        },
        /**
         * 获取组件的JSON格式表示
         * @returns {{name: *, type: *, className: *, template: *, data: *, api: *, adapter: *, isStatic: *, isCache: *, style: *, options: *}}
         */
        getJSON: function () {
            return {
                id: this.id,
        ...
        setVariable: function (obj) {
            this.variable = $.extend(this.variable, obj || {});
        },
        ...
        ajax: function () {
            ...
            //url注入变量
                url = mod.inject(_this.api, {query: $V.config.query}),
            ...
            if (!data) {
                $.ajax({
                    ...
                    success: function (msg) {
                        //调用数据处理函数
                        data = $V.config.adapters[_this.adapter](msg);

                        //保存到内存
                        mod.cache(url, data);

                        //保存到环境变量
                        _this.isCache && mod.cache(url, data, true);
                    },
                    error: function () {
                        TVUI.Util.log(arguments);
                    }
                });
            }
            return data;
        ...
        getInnerHTML: function () {
            ...
            var data = this.api ? this.ajax() : (this.data || []);
            ...
            return TVUI.View.render(this.template, this.viewData);
        },
        /**
         * 设置容器样式
         */
        setStyle: function () {
            var cls = ['v-mod', 'v-mod-' + this.type];
        ...
        render: function () {
            ...
            //是否已经在页面创建了HTML，如果未创建过，即加到页面上，如果已经创建过了，即更新内部HTML，并修改样式
            if (this.isAppend) {
                (t != 'List' && t != 'Nav' && t != 'Marquee') && this.el.html(this.getInnerHTML());
                this.setStyle();
            } else {
                //静态化是不需要创建html
                if (!this.isStatic) {
                    (t != 'List' && t != 'Nav' && t != 'Marquee') && this.el.html(this.getInnerHTML());
                    this.setStyle();
                    if (this.appendTo) $('#' + this.appendTo).append(this.el);
                    // $V.config.$main.append(this.el);
                }
                this.isAppend = true;
            }
         ...
         * @param methods
         */
        extendMethod: function (methods) {
            var _this = this;
            $.each(methods, function (i, n) {
                _this[n] = function () {
                    _this.model && _this.model[n].apply(_this.model, arguments);
                };
         ...
         * @param events
         */
        extendEvent: function (events) {
            var _this = this;
            if (_this.model) {
                $.each(events, function (i, n) {
                    _this.model.on(n, function () {
                        var arr = TVUI.Util.makeArray(arguments);
                        arr.unshift(n);
                        _this.fire.apply(_this, arr);
                    });
                });
            }
        },
        ...
        setCache: function (obj) {
            if (obj === null) {
                this.cache = {};
                TVUI.API.SysSetting.envJSON(this.id, null);
            } else {
                if (!this.cache) {
                    this.cache = {};
                }
                $.extend(this.cache, obj || {});
                TVUI.API.SysSetting.envJSON(this.id, this.cache);
            }
        },
        /**
         * 获取组件内部缓存
         ...
         */
        getCache: function () {
            return TVUI.API.SysSetting.envJSON(this.id) || {};
        },
    ...
    Base.include(TVUI.Base);
    mod.Base = Base;


    //Div组件
    mod.Div = TVUI.Class.create({
        _init: function () {
            this.type = 'Div';
        }
    }, mod.Base);
    ...
    mod.Slide = TVUI.Class.create({
        _init: function () {
            ...
            this.on('render', function () {
                ...
                //如实例已经创建，先要销毁，避免实例重复
                _this.model && _this.model.destroy();

                _this.model = new TVUI.Slide($.extend({
                    data: _this.viewData.data,
                    el: _this.el[0]
                }, _this.options || {}));

                _this.extendEvent(['change', 'select', 'focus', 'blur', 'active', 'unActive']);

            });
            this.extendMethod(['change', 'next', 'select', 'focus', 'blur', 'active', 'unActive']);
    ...
    mod.Button = TVUI.Class.create({
        _init: function () {
            ...
            this.on('render', function () {
                ...
                _this.model = new TVUI.Button($.extend({
                    el: _this.el[0]
                }, _this.options || {}));

                _this.extendEvent(['select', 'focus', 'blur', 'active', 'unActive', 'up', 'down', 'left', 'right']);
            });
            this.extendMethod(['select', 'focus', 'blur', 'active', 'unActive']);
        }

    }, mod.Base);

    mod.Marquee = TVUI.Class.create({
        _init: function () {
            this.type = 'Marquee';
            var _this = this;
            this.on('render', function () {
                ...
                _this.model && _this.model.destroy();
                _this.model = new TVUI.Marquee($.extend({
                    el: _this.el[0],
                    content: _this.getInnerHTML()
                }, _this.options || {}));

                _this.extendEvent(['start', 'stop']);

                _this.model.render();
            });

            this.extendMethod(['start', 'stop']);
        }
    }, mod.Base);


    //导航组件
    mod.Nav = TVUI.Class.create({
        _init: function () {
            ...
            this.on('render', function () {
                ...
                _this.setNavViewData();

                _this.model = new TVUI.Nav($.extend(true, {
                    container: _this.el[0],
                    data: _this.viewData.data,
                    template: _this.template
                }, _this.options || {}, {
                    panelOptions: {
                        focusIndex: _this.cache.focusIndex || 0
                    }
                }));

                _this.model.data.variable = _this.variable;
                _this.model.data.options = _this.model.options;

                _this.extendEvent(['focus', 'blur', 'select', 'active', 'unActive', 'leave', 'reset']);

                _this.model.render();

                var panel = _this.model.panel;

                //设置leave状态
                if (!panel._active && _this.cache.focusIndex >= 0) {
                    var cell = panel.indexToCell(_this.cache.focusIndex);
                    panel.fire('leave', 'right', cell);
                    panel.fire('initFixed', _this.cache.focusIndex);
                }

            });

            this.on('focus', function (i) {
                _this.setCache({focusIndex: i});
            });


            this.extendMethod(['focus', 'blur', 'select', 'active', 'unActive', 'leave', 'reset']);
        },
        setNavViewData: function () {
            var data = this.api ? this.ajax() : (this.data || {});
            this.viewData = {
                data: data,
                variable: this.variable
    ...
    mod.List = TVUI.Class.create({
        _init: function () {
            ...
            this.on('render', function () {
                ...
                _this.setPagerVariable(_this.cache.pageIndex || o.pageIndex, o.pageSize);

                _this.setListViewData();


                _this.model = new TVUI.List($.extend(true, {
                    container: _this.el[0],
                    data: _this.viewData,
                    template: _this.template,
                    total: _this.viewData.total
                }, o || {}, {
                    focusIndex: _this.cache.focusIndex || 0,
                    pageIndex: _this.cache.pageIndex || 0
                }));
                ...
                _this.model.on('setData', function () {
                    _this.model.data.variable = _this.variable;
                    _this.model.data.options = _this.model.options;
                });

                _this.extendEvent(['focus', 'blur', 'select', 'active', 'unActive', 'leave', 'reset', 'pager']);

                _this.model.render();
            });

            this.on('pager', function (pageIndex, type) {
                if (type > 0) {
                    var o = _this.options;
                    _this.setPagerVariable(pageIndex, o.pageSize);
                    _this.setListViewData();
                    _this.reset(_this.viewData, _this.viewData.total, pageIndex);
                    _this.autoFocus(type);
                }
            });

            this.on('focus', function (i) {
                _this.setCache({focusIndex: i, pageIndex: _this.model.pager.pageIndex});
            });

            this.on('unActive update', function () {
                _this.setCache(null);
            });


            this.extendMethod(['focus', 'blur', 'select', 'active', 'unActive', 'leave', 'reset', 'pager', 'autoFocus']);

        },
        setPagerVariable: function (pageIndex, pageSize) {
        ...
        setListViewData: function () {
            this.viewData = this.api ? this.ajax() : (this.data || {});
            $.extend(this.variable, {
                pageCount: Math.ceil(this.viewData.total / this.variable.pageSize)
            });
        },
        init: function () {
            this.parent.prototype.init.apply(this, arguments);
            var o = this.options;
            this.setPagerVariable(o.pageIndex, o.pageSize);
...
})($V, $V.modules, $, TVUI, window);
(function ($V, $, TVUI) {
    $V.page = {
        /**
         * 初始化页面, 在使用前先要执行初始化,执行完后会在window作用域下实例化TVUI.Page对象
         * 制作模式时,不会阻止页面默认按键默认行为,目的是为了在制作页面时允许键盘输入
         *
         * @param mode 模式: maker(制作模式), preview(预览模式), production(发布模式)
         * @param json 页面表示JSON
         */
        init: function (mode, json) {
            var options = {
                name: json.publishPath,
                preventDefault: mode !== 'maker' // 制作模式允许键盘输入文字
            };

            var root = document.getElementById(json.id) || document.createElement('div');
            root.className = 'app-root';
            root.id = json.id;
            document.body.appendChild(root);

            //如果是ETV平台,启动页面状态机,否则启动TVUI.Page
            if ($V.config.platform == 'ETV') {
                this.state = new TVUI.PageState({
                    el: root
                });
            } else {
                this.state = new TVUI.Page(options);
            }
            window[json.id] = this.state;
        },
        ...
        loadTheme: function (name) {
            ...
            link.setAttribute('href', $V.config.themes[name || $V.config.defaultTheme].href);
            $head.appendChild(link);
        },
        /**
         * 设置页面body的style
         * @param style 样式 style对象
         */
        ...
        getModulesJSON: function () {
            var json = {};
            $.each($V.modules.__modules, function (key, module) {
                json[key] = module.getJSON();
            });
            return json;
        },
        /**
         * 根据组件的JSON格式, 实例化组件并展示在页面上
         * 制作模式时允许拖拽修改位置和尺寸
         * @param modules  组件JSON
         * @param mode  模式
         */
        draw: function (modules, mode) {
            var self = this;
            $.each(modules || {}, function (key, val) {
                var mod = self.appendModule(val);
                if (window.maker && mode === 'maker') {
                    maker.util.setModuleDragAndResize(mod);
                }
            });
        },
        /**
        ...
        appendModule: function (module) {
            var Model = $V.modules[module.type];
            if (Model) {
                var mod = new Model(module);
                this.state.addPanel(mod);
                return mod;
            } else {
                alert('暂不支持 ' + module.type + ' 组件');
                return null;
            }
        }
