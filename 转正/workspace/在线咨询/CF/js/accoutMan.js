...
var domain = ["http://10.123.0.2:12690/","http://access.homed.me/"][isIP];
var userlist = domain+"account/user/get_list?deviceno={1}&devicetype={2}&pageidx={3}&pagenum={4}";
var userlogin = domain+"account/login?deviceno={1}&devicetype={2}&accounttype={3}&account={4}&accesstoken={5}&pwd=96e79218965eb72c92a549dd5a330112"; //密码是md5("111111")
...
function checkAccoutLogin(__callBackStr){	
	...
	loginCardId = getCaNo(); 
	iDebug("loginCardId>>"+loginCardId);
	if(typeof gotoPortal!="undefined"){
		accessToken = iPanel.eventFrame.access_token;
	}else{
		accessToken = getGlobalVar("accessToken");
	}
	//accessToken=R5A14D6CDU207EB000K53724E03IA401F20AM2FAF470W17989;loginCardId=8240004356202031
...
function getUserListInfo(){
	...
	accountAjaxObj.successCallback = function(xhr) {
		...
		iDebug("[getUserListInfo]responseText="+ responseText); //userid=50001008
...
function accoutLogin(__account,__type){
	...
	//http://10.123.0.2:12690/account/login?deviceno=8240004356202031&devicetype=2&accounttype=1&account=50001008&accesstoken=&
	...
	accountAjaxObj.failureCallback = function(xhr){
		iDebug("accoutLogin>>failureCallback")
	};
	accountAjaxObj.requestData();
}
