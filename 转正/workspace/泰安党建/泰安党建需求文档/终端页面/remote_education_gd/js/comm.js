...
var timer=-1;
function showTime(){//实时时间
	clearTimeout(timer);
	var now= new Date();//获取当前时间
	var year0 = (now.getFullYear()).toString(); //年
	var month0 = (now.getMonth()+1).toString(); //月
	var date0 = (now.getDate()).toString(); //日
	var hour0=(now.getHours()).toString();//时
	var minute0=(now.getMinutes()).toString();//分
	var second0=(now.getSeconds()).toString();//秒
