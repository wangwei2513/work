...
function checkAccoutLogin(__callBackStr){	
	...
	access_token = mySessionStorage.getItem("accessToken");
	...
	if(typeof(access_token)=="undefinded" || access_token == "" || access_token == null){
		...
		var tmpCardId = mySessionStorage.getItem("loginDeviceNo");
		if(loginCardId && (loginCardId != tmpCardId)){//卡号变化后，也重新登录
			mySessionStorage.removeItem("loginDeviceNo");
			mySessionStorage.removeItem("accessToken");
			getUserListInfo();	
		} else {
...
function registerUser(){
	...
	accountAjaxObj.url = registeruser.replace("{1}",loginCardId).replace("{2}","2").replace("{3}","1").replace("{4}",loginCardId).replace("{5}",hex_md5("111111")).replace("{6}",loginCardId).replace("{7}","1");
...
function loginCallBack(__accessToken){
	...
	if(typeof iPanel != "undefined"){
		iPanel.eventFrame.access_token = __accessToken;
	}else{
		mySessionStorage.setItem("accessToken",__accessToken);	
	}
	//mySessionStorage.setItem("loginDeviceNo",loginCardId);
