...
function globalVar(){
	this.getItem = function(_str){
		...
		try {
			/*if(typeof GlobalVarManager != "undefined") {
				result = GlobalVarManager.getItemValue(_str);
				if(!isValid(result)) {
					result = GlobalVarManager.getItemStr(_str);
				}
			} else */if(typeof iPanel != "undefined") {
				result = iPanel.getGlobalVar(_str);
			} else {
				result = sessionStorage.getItem(_str);
			}
		} catch(e) {
			var aCookie = document.cookie.split("; ");
			for (var i = 0,num=aCookie.length; i < num; i++) {
				var aCrumb = aCookie[i].split("=");
				if(escape(_str) == aCrumb[0]) {
	...
	this.removeItem = function(_str){
		try {
			/*if(typeof(GlobalVarManager) != "undefined") {
				GlobalVarManager.removeItem(_str);
			} else */if(typeof iPanel != "undefined") {
			...
			var cval = this.getItem(_str);
			document.cookie = _str + "=" + cval + "; expires=" + exp.toGMTString();
		}
	}
		...
		try {
			/*if(typeof GlobalVarManager != "undefined") {
				if(typeof _val == "string") {
					GlobalVarManager.setItemStr(_key,_val);
				} else {
					GlobalVarManager.setItemValue(_key, _val);
				}
			} else */if(typeof iPanel != "undefined") {
			...
			document.cookie = escape(_key) + "=" + escape(_val);
		}
	}
}
...
function AjaxClass(_url, _successCallback, _failureCallback, _completeCallback, _urlParameters, _callbackParams, _async, _charset, _timeout, _frequency, _requestTimes, _frame) {
	 ...
	 */
	if(typeof $G != 'undefined' && typeof $G.ajax != 'undefined'){
		return new $G.ajax(_url, _successCallback, _failureCallback, _completeCallback, _urlParameters, _callbackParams, _async, _charset, _timeout, _frequency, _requestTimes, _frame);
	}else{
		this.url = _url || "";
...
function jsonToString(_obj){
	...
	var THIS = this;
	iDebug("typeof(_obj)="+typeof(_obj)+" _obj="+_obj);
	switch (typeof(_obj)) {
	case "string":
